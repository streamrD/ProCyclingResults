...
Every results card links out to the full placings on ProCyclingStats through `getRaceResultsUrl()`: "Full results" beside the finish video on a one-day card, "Full stage results" under each stage panel, and "Full classification" at the foot of a jersey's top-five card. The addresses are the hand-verified `RACE_RESULT_SLUGS` map (page title without year to PCS race slug; `scripts/pcs-race-links.browser.js` checks it in a browser), with the PCS search page as the fallback for a race the map does not know.
...
- Runs the season calendar: the section is hidden until the hero's "Season Calendar" button (or a `#season-calendar` link) opens it and scrolls to it, a close button in its header hides it again, a "Full screen" button beside it pins the section over the whole window (Escape, the same button, closing it, or clicking a bar to jump to a card all put it back), series chips switch between three pre-rendered SVGs, each bar has a tooltip fed from its `data-tip-*` attributes, and clicking a bar whose card is hidden behind "Load more races" reveals it before the jump and flashes the card.
...
- Links every rider name to ProCyclingStats (opens in a new tab), and on pointer devices opens a small rider card after a 250 ms hover or on keyboard focus: flag and name, the season's wins and podiums as the site holds them (`riderSeasons`, an accent-folded index embedded as JSON before the client script, with the spellings that name one rider merged into a single tally published under each of them), and links to the rider's PCS page and exact Wikipedia article. The card is fixed-positioned on the body so the result cards' overflow clip cannot cut it off; it flips above the name when there is no room below, and closes on mouse-out, Escape, scroll or resize. Phones keep the plain link.
