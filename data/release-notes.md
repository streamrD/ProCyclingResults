...
- **The top five in every classification.** On a computer, rest the pointer on one of the classifications listed under a stage race's overall result and a card opens with the top five in it, the rider leading it first. Which classifications a race has is the race's own business: most run a general, points, mountains, young rider and team classification, while the Giro adds its intermediate sprint, Red Bull KM and breakaway prizes and the Tour de Pologne an active-rider jersey. Each card shows what its own classification is scored in — points for points and mountains, times and gaps for the general, young rider and team classifications, kilometres for the Giro's breakaway. Wikipedia updates these tables a stage after it updates the jerseys, so every card says which stage it counts to; on the day a jersey changes hands, that means the card can still have the previous holder on top. A classification whose table the article does not carry — the Tour de Pologne's Polish rider jersey, say — stays as it was, with nothing to open. Phones are unchanged.
...
- **Rider names are links.** Every rider name on the page links to their ProCyclingStats page, in a new tab. The addresses of all 517 names on the site were checked and corrected where PCS spells a name differently; a team named in a team time trial links to a PCS search instead.

- **The Montréal World Championships.** A new section below the WorldTour sections shows the four elite events, men's first: upcoming cards with date, local start time, distance and laps, then results (medallists, top five, nation flags, finish video and news) as soon as each event's Wikipedia page is filled in. On race days the site checks once a minute, as it does for a live Grand Tour.

- **A cancelled stage is no longer shown as a rider.** The Vuelta's cancelled stage 3 had been read as a rider named "Stage cancelled". It now appears in the stage strip as a struck-through number with the reason on hover.

- **The live race card says what kind of day it is.** On a rest day the pill reads "Rest day" and the card says when racing resumes; on other days it says whether the stage on the card finished today or the next stage is on the road, and the "Up next" row is labelled "Today" or "Tomorrow" with its date.
