...
- **Faster, steadier live results.** During a live race the site now refreshes its data itself every minute, so it no longer has to wait for a visitor to arrive before checking for a new stage result, and a visitor never lands on the "checking for fresh race data" screen mid-race. Today's Vuelta stage appeared about four minutes after the finish line.
- **The overall classification no longer disappears.** For a few minutes after each stage the official site publishes the stage result before the new overall standings. The card used to say the classification was not available; it now keeps showing the standings after the previous stage, labelled as such, until the new ones arrive.
