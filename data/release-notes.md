...
- **Rider names are links.** Every rider in a podium, a stage result, a general classification, a jersey row or a national championship card now links to that rider's page on ProCyclingStats, which opens on their results for this season and shows how they rank against everyone else, year by year. The name keeps its look, with a thin dotted underline, and opens in a new tab so the results stay where they are. The link goes through ProCyclingStats' search for the rider's name, which always lands; where two riders share a name you pick from a short list. A few riders link straight to their page.
