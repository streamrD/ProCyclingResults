...
- **A rider card on hover.** On a computer, rest the pointer on any rider's name for a moment and a small card opens under it: the rider's flag and name, what this site holds about their season (race wins, stage wins and podiums, where a race win is a one-day race or an overall classification), and links to their ProCyclingStats and Wikipedia pages. The Wikipedia link goes to the rider's exact article wherever the race pages we read link to it, so a name shared with other people, such as Ben Healy, lands on the cyclist and not on a list. Move away or press Escape to close it. The figures come only from results already on this site, so they say "on this site": every WorldTour and Worlds podium of the season, and stage wins from the stage results the page shows. Clicking the name still goes straight to ProCyclingStats, and on a phone, where there is no hover, nothing changes.

- **A cancelled stage is no longer shown as a rider.** The Vuelta's stage 3 was called off for the weather, and the route table says so where the winner would be. The site was reading those words as a rider named "Stage cancelled". The stage now appears in the stage strip as a struck-through number, and hovering it gives the reason from the race page. It is skipped when the card works out which stage is next.

- **Rider names are links.** Every rider in a podium, a stage result, a general classification, a jersey row or a national championship card now links to that rider's page on ProCyclingStats, which opens on their results for this season and shows how they rank against everyone else, year by year. The name keeps its look, with a thin dotted underline, and opens in a new tab so the results stay where they are. The link goes straight to the rider's page. All 517 names on the site today were checked against ProCyclingStats, and the ones whose address there differs from their name (a second surname, a different spelling) are corrected by hand; a rider new to the site gets the address built from their name, which is right for most. A team named in a team time trial goes to a search instead.

- **Worlds results, as soon as they exist.** The World Championships section now has a Results block. When an elite event finishes and its page on Wikipedia is filled in, the card moves from Upcoming to Results with the medallists, the top five with their times or gaps, the rider's nation flag, a "Watch the race finish" link found the same way as for other races, and the usual "Latest news" line. On a race day the site checks once a minute during Montréal racing hours, the same cadence it uses for a live Grand Tour, so the result should appear within a minute or two of the page being updated. Until it does, the event keeps its upcoming card, marked "Today". The men's cards come first, then the women's.
...
- **Latest news fits narrow cards.** On the three-column Recent Results cards the news line and its list ran past the edge of the card. Headlines now wrap to two lines and stay inside the card.
