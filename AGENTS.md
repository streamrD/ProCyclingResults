...
- Providers and Wikipedia spell the same rider differently, so `buildRiderSeasonIndex` settles one spelling per rider and `applyCanonicalRiderNames` writes it into the rows the race itself named — stage tables and winners, the general classification and its leader, the jersey holders, the route's winner column — before anything renders. The article title wins, then the spelling that kept its accents, then the one the page prints most; `mergeRiderNameVariants` first joins keys that are the same name plus one extra surname ("Enric Mas" and "Enric Mas Nicolau"), publishing one tally under both so a stale page still finds it. Season rows are deliberately left alone: Wikipedia wrote them and they are the spelling this settles on. Do not read a rider's name straight from a provider snapshot into new markup without going through this.
