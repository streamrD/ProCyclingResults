...
- `DATA-SOURCES.md` is our public statement to the sites we read from and the URL in our user agent. Any change to what the server fetches, how often, or how it caches must keep that document's table accurate and add a dated line to its review log. Measure requests per rebuild (count them per host in a VM harness, as on 2026-09-05) rather than guessing.
- Preserve current cache behavior unless the task clearly requires changing it. Live stage races intentionally use a shorter cache TTL than the default background race-data refresh, and during a live race the payload rebuilds itself on a timer (`scheduleLiveRaceRefresh`); requests never wait on a rebuild and the warm-up page is for an empty cache only (since 2026-09-05).
...
- Remember that the active product scope is Men's WorldTour, Women's WorldTour, and National Championships, plus the four elite UCI Road World Championships events (upcoming cards, then results). The Worlds are not on either WorldTour season page; `parseWorldChampionshipEliteEvents` reads them from the championship article's schedule tables and `enrichWorldChampionshipResults` reads each event's own page from its race day onward (never earlier: the pages do not exist before race week, and a missing page is retried every ten minutes at most). That page can lag the race by a day — the 2026 men's time trial had none the evening it was ridden — so a missing page falls back to the podium in the championship article's medal summary (`parseWorldChampionshipMedalSummary`). Their news and video searches are championship-shaped too, not race-name-shaped: `buildWorldChampionshipArticleQueries` and `matchesWorldChampionshipEvent` identify a story by the discipline, because a headline writes "World Championships" or "Worlds", never the card's full title. Their result tables list a nation instead of a team, so `parseWorldChampionshipEventResult` is their parser and the shared one-day parser skips them. From a week before the first elite event to three days after the last, the section and its menu button lead the page (`isWorldChampionshipWeek`); the rest of the year they sit after the WorldTour. See "World Championships" under "Data Source Cross-Reference" in `handoff.md`.
- Every card also links out to the full placings on ProCyclingStats (`getRaceResultsUrl`): "Full results" beside the finish video on a one-day card, "Full stage results" under each stage, and "Full classification" at the foot of a jersey's top-five card. The addresses come from `RACE_RESULT_SLUGS`, keyed by the page title without its year and verified in a browser with `scripts/pcs-race-links.browser.js` (a wrong slug answers HTTP 500 with an empty body); a race missing from the map goes to the PCS search page. When the calendar gains a race, add its slug and re-run the check. The team classification has no PCS page, so its card carries no link.
- Rider names are links to ProCyclingStats (`getRiderProfileUrl`): the address is built from the name by `buildRiderSlug`, corrected by `RIDER_PROFILE_URLS` where PCS spells it differently, and sent to the PCS search page for team names and lone surnames. PCS blocks the server (Cloudflare), so nothing about it can be checked from code: `scripts/pcs-rider-links.browser.js` is pasted into a browser console on a PCS page to verify a batch of names. We link to PCS and never fetch from it; keep `DATA-SOURCES.md` true to that.
- The rider hover card reads the `riderSeasons` index (`buildRiderSeasonIndex`, keyed by `foldRiderKey` because providers and Wikipedia spell riders differently) embedded as JSON before the client script. Wins and podiums each count one-day, overall and stage results together; stage results count only for races whose stage history the payload holds. The card's Wikipedia link is the article title the wikitext linked (`pageTitle` on standing entries and `winnerPageTitle` on season rows), falling back to Wikipedia's go-to-title search.
- Providers and Wikipedia call the same rider different things, so `buildRiderSeasonIndex` settles one spelling per rider and `applyCanonicalRiderNames` writes it over every row that names one — stage tables and winners, `latestStage`, the general classification and its leader, the jersey holders, the route's winner column, top fives, and the season rows — before anything renders. Two spellings are one rider when they link the same Wikipedia article (`groupRiderNameVariants`; the strongest signal, and the only one that catches a shortened first name, "Kim" beside "Kimberley" Le Court-Pienaar) or when one is the other plus a single extra name at either end ("Enric Mas"/"Enric Mas Nicolau", "Oscar Onley"/"Edgar Oscar Onley"). The merged tally is published under every spelling so a page cached before a rename still finds it. The winning name is the most-linked article title (`pickMostLinkedTitle` — never the first one met: Wikipedia links a rider under several titles after a rename), then the spelling that kept its accents, then the one the page prints most. Do not read a rider's name straight from a provider snapshot into new markup without going through this, and **anything that mutates a race after `buildRaceData` returns must re-run `applyCanonicalRiderNames` or it will undo the settling** — `applyLateOfficialSnapshots` does, and that was the last of four rounds of this bug. See "Added 2026-09-08" in `handoff.md`.
- A route-table winner cell that says "Stage cancelled" is a cancellation, not a rider (`parseRouteStageCancellation`); the stage keeps its route entry with `cancelled: true` and a note, renders as a struck-through chip, and is skipped as "next". `isPlausibleRiderName` rejects the phrase from any source.
...
- The season year is not a constant to edit. `SEASON_YEAR` starts at `FIRST_SEASON_YEAR` and `resolveSeasonYear` (top of `buildRaceMetadata`) moves it on a week before the next season's first race, read from that season's WorldTour pages (`probeSeasonOpening`); it never steps back within a process. Everything keyed to one season derives from it — `getSeasonSources`, `getWorldChampionshipsPageTitle`, `getNationalChampionshipsSource`, the typical nationals windows, and `NATIONAL_CHAMPIONSHIP_EVENT_METADATA` (applied only when its date is in the active season). Race-specific fixes key on the page title ("2026 Giro d'Italia") and so stop applying to the next edition by design. The rollover still needs a person to look at the site the week it happens: the Cyclingnews address is a guess from the pattern, and the Worlds parser has only met the 2026 article.
- Between the last WorldTour race and the rollover, the header is the Grupetto Committee's closing note, headlined "Thank you, <year>" (`buildSeasonCloseout` → `seasonCloseout` in the payload, rendered by `buildSeasonCloseoutHero`). Every figure in it is read from the season calendar; the first season counts from launch day (`SEASON_CLOSEOUT_LAUNCH`), and the next season's first-results date is shown only once its pages carry one — until then it says the month, never a guessed day. The committee's winter lines are `GRUPETTO_WINTER_ROSTER`, chosen by the maintainer on 2026-09-15.
...
- Every results card ends with a "Latest news" line (`buildRaceNewsMarkup`) that opens the race's eight stories in place; it must never fetch article pools at build time for recent cards — placeholders are filled by `/api/race-news` as cards scroll into view. A cached pool older than its cache window counts as cold (`peekRaceArticlePool`), which is the only thing that makes a live race's stories refresh; until 2026-09-12 a pool was fetched once per process and never again. The old "Race Coverage" block and `/api/competition-coverage` were retired on 2026-09-05 (`archive/race-coverage-block.js`); do not bring them back unless asked. See "Stage Results Feature Map" in `handoff.md`.
...
- Hovering a classification in that list opens its top five, read from the article's "Classification standings" section (`extractClassificationStandings`, both the captioned-wikitable and `{{cyclingresult}}` forms, keyed the way the leadership columns are keyed). What it prints is what the classification is scored in — points, time, or the Giro's kilometres — taken from the table's own column heading; never relabel a time gap as a point total. These tables trail the leadership table by a stage during a live race, so the card is dated by its own caption, and where an article writes a classification more than once the newest table wins. See item 7a of "Stage Results Feature Map" in `handoff.md`.
...
- `/release-notes` and `/about` render `data/release-notes.md` and `data/about.md`. When shipping a user-visible change, add a dated entry to the release notes in plain language, and keep it short: a bold lead of a few words, then one or two sentences. Say what changed for the reader, not how it was found or fixed (the maintainer asked for this on 2026-09-12; the handoff is where the detail goes). The maintainer can also edit both pages in place on the live site (`SITE_EDIT_TOKEN`, optional `GITHUB_CONTENT_TOKEN` to commit), so pull before editing those files by hand.
...
- When a parser must meet a page that does not exist yet (a new edition, race day
  tomorrow), run it against every earlier edition's page first. Different editors write
  different layouts; on 2026-09-19 the Worlds parser read 2024-25 cleanly but missed
  the top five on every 2019-23 time trial.
- A test that passes a fixed date to one function while the code under test reads the
  real clock will start failing on its own once the calendar moves on. Pass the same
  clock to both (`getCompetitionGroups(data, now)` is the pattern).
